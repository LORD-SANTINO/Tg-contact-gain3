...
async def handle_upload_vcf(event):
    ...
    # Store the callback to handle the next message from this user
...
async def handle_vcf_file(event):
    # Check if this is a document with .vcf extension
    if event.document and any(attr.endswith('.vcf') for attr in getattr(event.document, 'attributes', [])):
    ...
    client.remove_event_handler(handle_vcf_file)
